      ...
      var cidArray = response.split(',');
       jQuery('.classchk').removeAttr('checked', 'checked');
            ...
            jQuery.each(cidArray, function(index, value) { 
                jQuery('.classid-'+value).attr('checked', 'checked');
            });
...
/*
 jQuery("#usergroup").change(function(){
      ...
      var gid = jQuery("#usergroup").val();
    var myData = {'action':'get_selected_class','gid':gid}
        jQuery.post(ajax_object.ajax_url,myData,function(result){
          
          if(result == 1)
          {
              jQuery(formid).html('<p class="bsuccess">Updated Successfully</p>');
          }
          else
          {
            jQuery(formid).html('<p class="bsuccess fail"> Not available booking on this date</p>');  
          }
        });    
    return false;
  });
